function dataSet_smooth=smoothing_NBIoT(dataSet, dataSet_interp, interpolationOn)
if(interpolationOn)
...
for q=1:length(desiredParamVector)
    ...
    for i = 1:n
        ...
        thisNPCI = thisMatrix(:,[1 2 8]);             % unique NPCIs for point under consideration
        ...
        for j = idx
            ...
            for k = 1:size(uniquePCIsInPoint,1)
                ...
                thisparamData = thisparamDataFull(ismember(thisparamDataFull(:,[1 2 8]),uniquePCIsInPoint(k,:),'rows'),desiredParam);
        ...
        for j = 1:size(paramData,1)
            ...
            thisMatrix(isequal(thisMatrix(:,[1 2 8]),paramData(j,2:4)),desiredParam) = paramData_mean(isequal(paramData_mean(:,1:3),paramData(j,2:4)),4);
