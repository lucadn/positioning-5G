...
temp_RFP = cellfun(@(a){{a(:,[1 2 8])}},struct_RFP(:,3));
RFP_NPCI_ID_Op=cell2mat(cellfun(@(x) cell2mat(x),temp_RFP,'un',0));
temp_TP = cellfun(@(a){{a(:,[1 2 8])}},struct_TP(:,3));
TP_NPCI_ID_Op=cell2mat(cellfun(@(x) cell2mat(x),temp_TP,'un',0));
uniqueNPCIs = unique([TP_NPCI_ID_Op; RFP_NPCI_ID_Op], 'rows');

%Restrict the set of unique PCIs to the set of selected operators
opIndexes=zeros(size(uniqueNPCIs,1),1);
for i=1:length(operatorChoiceVector)
    opIndexesTemp=(uniqueNPCIs(:,3)==operatorChoiceVector(i));
    opIndexes=or(opIndexesTemp,opIndexes);
end
uniqueNPCIs=uniqueNPCIs(opIndexes,:);

M_RFP = (missRefValue)*ones(size(struct_RFP,1), size(uniqueNPCIs,1)); % Reference Point matrix (Reference Point x NPCI)
M_TP = (missRefValue)*ones(size(struct_TP,1), size(uniqueNPCIs,1));   % Test Point matrix (Test Point x NPCI)
idx2=zeros(size(struct_RFP,1), size(uniqueNPCIs,1));
...
for i = 1:size(struct_RFP,1)
    ...
    actualNPCIs=potentialNPCIs(~isnan(RFP_mat(logical(lib),RF_param)));
...
for i = 1:size(struct_TP,1)
    ...
    L(i,:) = s;
    bin = unique(L(i,:));  %% number of unique NPCIs in common
...
RFP_coordinates = cell2mat(struct_RFP(:,1:2));
TP_coordinates = real_position;
