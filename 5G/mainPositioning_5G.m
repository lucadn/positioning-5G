...
activeCampaigns=1:6;
dataFileName=sprintf('Campaign_data_5G');
...
if(fExists~=2)
    ...
    createDataFiles_5G(dataFileNameRoot,dataFileName,activeCampaigns,interpolationOn,smoothingOn);
...
operatorChoice=1:2;%Vector indicating which operators should be used.
kMax=40; %maximum number of neighbours used in WkNN
nRuns = 20; % number of runs to be excecuted
...
for pCount=1:length(RF_param_vector)
    ...
    try % Initialization
        ppm = ParforProgressStarter2('5G Positioning', nRuns, 0.1, 0, 1, 1);
    ...
    parfor k = 1:nRuns
    %for k = 1:nRuns
        [average_error_tmp{1,k},percLocated(1,k),TPs_located{1,k}, TP_est_location{1,k}]=Wei_Cov_strategy(dataSet, dataSet_smooth, operatorChoice, RF_param, kMax);
        ppm.increment(k);
    end
